...
const fastapi_topics = [
    {
        ...
        "Middleware for Logging/Tracking",
        "Error Handling",
    ...
    {
        title: "Training Material – Fast API Learning Guide",
        children: [],
    },
];


const FastApiSidebar = ({onImageClick}: {onImageClick: () => void}) => {
    const selected = useFastApiStore((state) => state.selected);
    const setSelected = useFastApiStore((state) => state.setSelected);
    const setTitle = useFastApiStore((state) => state.setTitle);
    const [openSections, setOpenSections] = useState<Record<string, boolean>>({
        "Best Practices : Thinking in FastAPI": false, // default open
    });
    ...
    return (
        ...
        <div className="relative w-full max-h-[80vh] overflow-y-auto">
            ...
            {fastapi_topics.map(({ title, children }) => (
                <li key={title}>
                <div
                    onClick={() =>{
                        toggleSection(title);
