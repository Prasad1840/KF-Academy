...
const react_topics = [
    ...
    {
        title: "Training Material – React Learning Guide",
        children: [],
    },
];


const ReactSidebar = ({onImageClick}: {onImageClick: () => void}) => {
    const selected = useReactStore((state) => state.selected);
    const setSelected = useReactStore((state) => state.setSelected);
    const setTitle = useReactStore((state) => state.setTitle);
    const [openSections, setOpenSections] = useState<Record<string, boolean>>({
        "Best Practices : Thinking in React": false, // default open
    });
    ...
    return (
        ...
        <div className="relative w-full max-h-[80vh] overflow-y-auto">
            ...
            {react_topics.map(({ title, children }) => (
                <li key={title}>
                <div
                    onClick={() =>{
                        toggleSection(title);
